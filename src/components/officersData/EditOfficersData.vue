<template>
    ...
    >
      <template #modal-title>
        <h2 class="m-0">OFFICER BASIC DATA DETAIL</h2>
      </template>
      <b-card class="mb-3">
        ...
        <b-form @submit.prevent>
          ...
          <b-form-group class="mt-2 text-right">
            ...
            <b-button
              ...
              v-if="hasPermission('update_officer_data')"
            >
              Update
            </b-button>
          </b-form-group>
  ...
  export default {
    ...
    mounted() {
      ...
      if(this.item){
         ...
         if(this.item.form_filled_by == 'self'){
          this.personField = true;
         }
         else{
          this.personField = false;
         }
        if (this.item.present_address === this.item.permanent_address) {
        ...
        this.cnic = this.item.cnic;
        this.mobileNo = this.item.mobile;
        if(this.item.svc_number) {
          this.selectedType = this.item.prefix_data;
          if (this.item.svc_number.includes('-')) {
            this.split_svc_number = this.item.svc_number.split('-');
            this.prefix = this.split_svc_number[0];
            this.enteredNumber = this.split_svc_number[1];
          }
          else{
            this.enteredNumber = this.item.svc_number;
          }  
        }
        this.wardsOf = this.item.wards
          ? this.wardsName[this.item.wards]
          : "";
        this.pmaCourse = this.item.pma_course;
        this.whatsappNo = this.item.whatsapp_no;
        this.rank = this.item.rank_data;
        this.unit = this.item.unit_data;
        this.formation = this.item.formation_data;
        this.presentAddress = this.item.present_address;
        this.permanentAddress = this.item.permanent_address;
        // this.sameAsPresentAddress = this.item.same_as_present_address;
    ...
    methods: {
      ...mapActions({
        ...
        updateOfficersData: "appData/updateOfficersData",
      ...
      async submit() {
        ...
        try {
          ...
          //   this.sameAsPresentAddress = false;
          // }
          const res = await this.updateOfficersData({
            payload: {
              reason: this.reason,
              name: this.name,
          ...
          if (res.status === 200) {
            this.$swal({
              title: "Officers Data Update successfully",
              icon: "success",
            });
            this.show = false;
            this.reset();
            this.$nextTick(() => {
              this.$bvModal.hide("officers-edit-modal");
            });
